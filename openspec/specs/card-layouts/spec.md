# card-layouts Specification

## Purpose
TBD - created by archiving change card-layouts-and-imagery. Update Purpose after archive.
## Requirements
...
- **THEN** the user SHALL be able to open the glass size selection dialog from the card
- **THEN** selecting a glass size SHALL update the display and persist exactly as in the landscape layout

#### Scenario: Detected beer shown in every layout
- **WHEN** any layout is active
- **THEN** the beer SHALL be determined by the `keg-auto-detection` rules rather than by a per-layout control
- **THEN** the no-keg-detected state SHALL be surfaced in every layout, not only in landscape
